...
//				pw.initialize();
	...
	class PlayerWindow extends JFrame {
		...
		public void initialize() {
			if (player != null) {
				add(new PlayerPanel(player), BorderLayout.CENTER);
			}
		}
...
//			prUsage();
//
//		Receiver avReceive = new Receiver(argv);
...
//		if (args.length == 0)
		{
			args = new String[]{"224.1.1.1/22222"};
		}
		Receiver avReceive = new Receiver(args);
		
		avReceive.pw.setVisible(true);
		if (!avReceive.initialize()) {
			System.err.println("Failed to initialize the sessions.");
			System.exit(-1);
		}
...
//			public void run() {
//				try {
//					Receiver window = new Receiver(args);
